...
class PersonDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # YOLO 감지 실행 - person, sports ball 감지
            results = self.model.predict(
                source=cv_image,
                classes=[0, 32],  # 0: person, 32: sports ball (COCO)
                conf=0.3,  # 감지를 위한 신뢰도 임계값
                verbose=False,
                show=False,
                save=False
            )
            ...
            for r in results:
                ...
                for box in r.boxes:
                    # class 확인 (COCO dataset에서 person=0, sports ball=32)
                    ...
                    if cls_id in (0, 32):
                        ...
                        label = "PERSON" if cls_id == 0 else "SPORTS BALL"
                        self.get_logger().info(
                            f"✅ {label} DETECTED! Confidence: {confidence:.2f}")
